...
import * as compactionRuntime from "../compaction.js";
          ...
          if (pruned.droppedChunks > 0) {
            ...
            if (pruned.droppedMessagesList.length > 0) {
              try {
                ...
                droppedSummary = await compactionRuntime.summarizeInStages({
        ...
        try {
          const historySummary =
            messagesToSummarize.length > 0
              ? await compactionRuntime.summarizeInStages({
                  messages: messagesToSummarize,
                  model,
                  apiKey,
                  signal,
          ...
          if (preparation.isSplitTurn && turnPrefixMessages.length > 0) {
            const prefixSummary = await compactionRuntime.summarizeInStages({
