...
export type ResolvedMemorySearchConfig = {
  ...
  /** SQLite store config — dead code in ClawMongo (MongoDB uses its own storage). */
  store: {
    driver: "sqlite";
    path: string;
    vector: {
...
function mergeConfig(
  ...
  const sessionMemoryDefault = memoryBackend === "mongodb";
  const sessionMemory =
  ...
  return {
    ...
    local,
    store: { driver: "sqlite" as const, path: "", vector: { enabled: false } },
