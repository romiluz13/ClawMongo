...
export class MongoDBMemoryManager implements MemorySearchManager {
  ...
  async search(
    ...
    // Normalize scores to [0,1] for consistent cross-source comparison.
    ...
    const normalized = normalizeSearchResults(legacyResults, legacyMethod);

    const sorted = normalized.toSorted((a, b) => b.score - a.score);

    // Deduplicate results by content — keep highest-scoring on duplicate
    const deduped = deduplicateSearchResults(sorted);
    const dedupCount = sorted.length - deduped.length;
    if (dedupCount > 0) {
