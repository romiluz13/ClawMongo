...
describe("mergeContiguousChunks", () => {
  ...
  it("does not merge non-conversation results (episodes, kb, structured)", () => {
    const results = [
      makeResult({ path: "episode/a", source: "reference", score: 0.8, snippet: "ep1" }),
      makeResult({ path: "episode/b", source: "reference", score: 0.6, snippet: "ep2" }),
    ];
    const merged = mergeContiguousChunks(results);
    expect(merged).toHaveLength(2);
  });
  ...
  it("does not merge results without sessionId (passes through unchanged)", () => {
    const results = [
      makeResult({ path: "events/a", timestamp: new Date("2026-01-01T00:01:00Z"), score: 0.8 }),
      makeResult({ path: "events/b", timestamp: new Date("2026-01-01T00:02:00Z"), score: 0.6 }),
    ];
    const merged = mergeContiguousChunks(results);
    expect(merged).toHaveLength(2);
  });
  ...
  it("handles mixed conversation and non-conversation results", () => {
    const results = [
      ...
      makeResult({ path: "kb/doc1", source: "reference", score: 0.7, snippet: "kb result" }),
    ];
    const merged = mergeContiguousChunks(results);
