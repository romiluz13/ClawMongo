...
function mergeSourceEventIds(...docs: Array<Document | null | undefined>): string[] | undefined {
...
async function settled<T>(label: string, fn: () => Promise<T>): Promise<T | null> {
  try {
    return await fn();
  } catch (error) {
...
function toEntityEvidence(doc: Document): MemoryDiscoveryProjectionEvidence {
  return {
    ...
    ...(typeof doc.scope === "string" ? { scope: doc.scope as MemoryScope } : {}),
    ...(typeof doc.scopeRef === "string" ? { scopeRef: doc.scopeRef } : {}),
    ...(getSourceEventIds(doc) ? { sourceEventIds: getSourceEventIds(doc) } : {}),
  };
}
...
function toRelationEvidence(
  ...
  const fromName =
    entityNames.get(String(doc.fromEntityId ?? "")) ?? String(doc.fromEntityId ?? "");
  const toName = entityNames.get(String(doc.toEntityId ?? "")) ?? String(doc.toEntityId ?? "");
  return {
...
function toStructuredEvidence(doc: Document): MemoryDiscoveryProjectionEvidence {
  return {
    ...
    ...(doc.updatedAt instanceof Date ? { timestamp: doc.updatedAt } : {}),
    ...(doc.supersededAt instanceof Date ? { timestamp: doc.supersededAt } : {}),
    ...(typeof doc.scope === "string" ? { scope: doc.scope as MemoryScope } : {}),
    ...(typeof doc.scopeRef === "string" ? { scopeRef: doc.scopeRef } : {}),
    ...(getSourceEventIds(doc) ? { sourceEventIds: getSourceEventIds(doc) } : {}),
  };
    ...
    ...(current?.updatedAt instanceof Date
      ...
      : revision.supersededAt instanceof Date
        ...
        : revision.updatedAt instanceof Date
          ...
          : {}),
    ...(typeof revision.scope === "string" ? { scope: revision.scope as MemoryScope } : {}),
    ...(typeof revision.scopeRef === "string" ? { scopeRef: revision.scopeRef } : {}),
...
function toProcedureEvidence(doc: Document): MemoryDiscoveryProjectionEvidence {
  const steps = Array.isArray(doc.steps)
    ? doc.steps.filter((value): value is string => typeof value === "string").slice(0, 2)
    : [];
  return {
...
function toEpisodeEvidence(doc: Document): MemoryDiscoveryProjectionEvidence {
  return {
    ...
    ...(doc.timeRange?.end instanceof Date ? { timestamp: doc.timeRange.end } : {}),
    ...(typeof doc.scope === "string" ? { scope: doc.scope as MemoryScope } : {}),
    ...(typeof doc.scopeRef === "string" ? { scopeRef: doc.scopeRef } : {}),
    ...(getSourceEventIds(doc) ? { sourceEventIds: getSourceEventIds(doc) } : {}),
  };
...
function toEventEvidence(doc: Document): MemoryDiscoveryProjectionEvidence {
  return {
    title: typeof doc.role === "string" && doc.role.trim() ? `${doc.role} event` : "event",
    ...
    ...(typeof doc.scope === "string" ? { scope: doc.scope as MemoryScope } : {}),
    ...(typeof doc.scopeRef === "string" ? { scopeRef: doc.scopeRef } : {}),
    ...(typeof doc.eventId === "string" ? { sourceEventIds: [doc.eventId] } : {}),
  };
}
...
async function buildEntityBrief(params: {
  ...
  const entities = await settled("entity-brief.entities", () =>
    entitiesCollection(db, prefix)
      .find(
        {
          ...scopeFilter,
          ...(regex ? { $or: [{ name: regex }, { aliases: regex }] } : {}),
        },
        { sort: { updatedAt: -1 } },
      )
      .limit(maxItems)
  ...
  const matchedIds = matchedEntities.map((entity) => String(entity.entityId ?? ""));
  const relationDocs =
    matchedIds.length === 0
      ? []
            ...
            .find(
              {
                ...scopeFilter,
                state: "active",
                $or: [{ fromEntityId: { $in: matchedIds } }, { toEntityId: { $in: matchedIds } }],
              },
              { sort: { updatedAt: -1 } },
            )
            .limit(maxItems)
      ...
      .find(
        {
          ...
          state: "active",
          ...(regex
            ? { $or: [{ key: regex }, { value: regex }, { context: regex }, { tags: regex }] }
            : {}),
        },
        { sort: { updatedAt: -1 } },
      )
...
async function buildTopicBrief(params: {
  ...
  const partial = [episodes, structured, procedures].some((result) => result === null);
  return {
...
async function buildWhatChanged(params: {
  ...
  const { db, prefix, agentId, scope, scopeRef, query, maxItems, timeRange } = params;
  const regex = buildQueryRegex(query);
  const scopeFilter = { agentId, scope, scopeRef };
  ...
  for (const doc of currentStructured ?? []) {
    currentStructuredByKey.set(`${String(doc.type ?? "")}::${String(doc.key ?? "")}`, doc);
  }
  ...
  const procedureDocs = pickLatestDocuments(procedures ?? [], {
    identity: (doc) => (typeof doc.procedureId === "string" ? doc.procedureId : undefined),
    timestamp: (doc) => (doc.updatedAt instanceof Date ? doc.updatedAt : undefined),
  });
      ...
      .find(
        {
          ...
          updatedAt: dateFilter,
          ...(regex
            ? { $or: [{ fromEntityId: regex }, { toEntityId: regex }, { type: regex }] }
            : {}),
        },
        { sort: { updatedAt: -1 } },
      )
  ...
  const relationDocs = pickLatestDocuments(relations ?? [], {
    identity: (doc) => {
      const fromEntityId = typeof doc.fromEntityId === "string" ? doc.fromEntityId : undefined;
      const type = typeof doc.type === "string" ? doc.type : undefined;
      const toEntityId = typeof doc.toEntityId === "string" ? doc.toEntityId : undefined;
  ...
  if (relationDocs.length > 0) {
    sections.push({
      ...
      evidence: relationDocs.map((doc) => toRelationEvidence(doc, new Map<string, string>())),
    });
  }
  if ((events ?? []).length > 0) {
    sections.push({
  ...
  const partial = [revisions, currentStructured, procedures, relations, events].some(
    (result) => result === null,
  );
...
async function buildContradictionReport(params: {
  ...
  if ((relations ?? []).length > 0) {
    sections.push({
      ...
      evidence: (relations ?? []).map((doc) => toRelationEvidence(doc, new Map<string, string>())),
    });
  }

  const partial = [structured, procedures, relations].some((result) => result === null);
  const evidenceSections = capSections(sections, maxItems);
  const evidence = flattenEvidence(evidenceSections);
  return {
    title: query?.trim() ? `Contradiction report for ${query.trim()}` : "Contradiction report",
    summary:
      evidence.length > 0
...
function projectionKindRequiresQuery(kind: MemoryDiscoveryProjectionKind): boolean {
