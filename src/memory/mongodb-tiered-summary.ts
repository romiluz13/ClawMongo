...
export function withTieredSummaries(
  ...
  return async (events) => {
    ...
    try {
      ...
      if (!tiered) {
        log.warn("tiered summary LLM response could not be parsed, using base summary only");
        return base;
      }
      ...
      log.warn("tiered summary generation failed, using base summary only", { error: err });
      return base;
    }
  };
}
