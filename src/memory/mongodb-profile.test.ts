...
describe("mongodb-profile", () => {
  ...
  it("synthesizeProfile groups structured memory by type via $facet", async () => {
    ...
    const facetResult = [
      {
        preferences: [{ key: "pref1", value: "dark mode", salience: "normal", updatedAt: now }],
        decisions: [{ key: "dec1", value: "use TypeScript", salience: "high", updatedAt: now }],
        facts: [{ key: "fact1", value: "project started 2026", salience: "low", updatedAt: now }],
        todos: [{ key: "todo1", value: "add tests", salience: "critical", updatedAt: now }],
      },
    ];
  ...
  it("synthesizeProfile limits items per type to maxPerType", async () => {
    ...
    for (const type of ["preferences", "decisions", "facts", "todos"]) {
      ...
      const limitStage = subPipeline.find((s: Document) => s.$limit !== undefined);
  ...
  it("synthesizeProfile returns top entities by relation count", async () => {
    ...
    expect(profile.topEntities[1].name).toBe("ClawMongo");
