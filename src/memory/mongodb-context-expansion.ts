...
export async function expandSearchContext(params: {
  ...
  const expandable: Array<{ result: MemorySearchResult; sessionId: string; timestamp: Date }> = [];
  const existingPaths = new Set(results.map((r) => r.path));

  for (const r of results) {
    if (r.path.startsWith("events/") && r.sessionId && r.timestamp) {
      expandable.push({ result: r, sessionId: r.sessionId, timestamp: r.timestamp });
    }
  }
  ...
  const bySession = new Map<string, Array<{ result: MemorySearchResult; timestamp: Date }>>();
  ...
  for (const [sessionId, items] of bySession) {
    ...
    for (const item of items) {
      ...
      for (const event of sessionEvents) {
        const eventTs = (event.timestamp as Date).getTime();
      ...
      for (const event of [...nearestBefore, ...nearestAfter]) {
        ...
        const snippet = renderEventChunkText({ role: event.role, body: event.body });
