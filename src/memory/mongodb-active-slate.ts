...
import type { MemoryActiveSlate, MemoryActiveSlateItem, MemoryActiveSlateKind } from "./types.js";
...
function clampMaxItems(maxItems?: number): number {
  ...
  return Math.max(1, Math.min(ACTIVE_SLATE_MAX_ITEMS, Math.floor(maxItems ?? 0)));
}
...
function sortActiveStructuredDocs(docs: Document[]): Document[] {
  return [...docs].toSorted((left, right) => {
    const salienceDelta = salienceRank(left.salience) - salienceRank(right.salience);
    if (salienceDelta !== 0) {
...
function toStructuredItem(
  doc: Document,
  kind: Extract<MemoryActiveSlateKind, "active-critical" | "decision" | "current-state">,
...
function toAnchorItem(doc: Document): MemoryActiveSlateItem {
  return {
    kind: "recent-anchor",
    source: "conversation",
    title: typeof doc.role === "string" && doc.role.trim() ? `${doc.role} anchor` : "recent anchor",
...
export async function hydrateActiveSlate(params: {
  ...
  try {
    const [activeCriticalDocs, procedureDocs, durableDocs, anchorDocs] = await Promise.all([
          ...
          .find(
            {
              ...
              type: { $in: ["todo", "fact", "project", "architecture", "custom", "decision"] },
              ...nonExpiredFilter,
            },
            { sort: { updatedAt: -1 } },
          )
          ...
          .find(
            {
              ...scopeFilter,
              state: "active",
              type: { $in: ["decision", "project", "fact", "architecture", "custom"] },
              ...nonExpiredFilter,
            },
            { sort: { updatedAt: -1 } },
          )
      ...
      settled("recent-anchors", () =>
        eventsCollection(db, prefix)
          .find(scopeFilter, { sort: { timestamp: -1 } })
          .limit(sourceLimit)
          .project({
            eventId: 1,
            role: 1,
    ...
    const candidateArrays = [
      ...
      (durableDocs ?? []).map((doc) =>
        toStructuredItem(doc, doc.type === "decision" ? "decision" : "current-state"),
      ),
    ...
    const partial = [activeCriticalDocs, procedureDocs, durableDocs, anchorDocs].some(
      (result) => result === null,
    );
    const candidateCount = candidateArrays.reduce((total, group) => total + group.length, 0);
