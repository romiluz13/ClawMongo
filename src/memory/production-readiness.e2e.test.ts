...
const describeIfMongo = TEST_URI ? describe : describe.skip;
  ...
  if (plan.stages) {
    for (const stage of plan.stages) {
      for (const val of Object.values(stage)) {
        if (typeof val === "object" && val !== null && hasCollScan(val as Document)) {
          return true;
        }
      }
    }
...
const testSummarizer: EpisodeSummarizer = async (events) => {
  ...
  for (const e of events) {
    ...
    if (e.body.includes("bug") || e.body.includes("fix") || e.body.includes("issue")) {
      topics.add("bugfix");
    }
...
describeIfMongo("Production-Readiness E2E: Operational Quality Validation", () => {
  ...
  describe("Phase 1: Foundation Data", () => {
    it("seeds 20 events across 3 sessions", async () => {
      ...
      const count = await eventsCollection(db, PREFIX).countDocuments({ agentId: AGENT_ID });
      expect(count).toBe(
        SESSION_1_EVENTS.length + SESSION_2_EVENTS.length + SESSION_3_EVENTS.length,
      );
    });
    ...
    it("extracts entities from conversation content", async () => {
      ...
      const entities = await entitiesCollection(db, PREFIX).find({ agentId: AGENT_ID }).toArray();
    ...
    it("creates structured memory entries", async () => {
      ...
      const count = await structuredMemCollection(db, PREFIX).countDocuments({ agentId: AGENT_ID });
      expect(count).toBeGreaterThanOrEqual(3);
    });

    it("creates entity relations", async () => {
      const relCount = await relationsCollection(db, PREFIX).countDocuments({ agentId: AGENT_ID });
  ...
  describe("Phase 2: Write-Path Quality", () => {
    it("emits entity-extraction telemetry", async () => {
      ...
      const extractionDocs = await telCol
        .find({ "meta.operation": "entity-extraction", "meta.agentId": AGENT_ID })
        .toArray();
    ...
    it("records extraction method in telemetry", async () => {
      ...
      const docs = await telCol
        .aggregate([
          { $match: { "meta.operation": "entity-extraction", "meta.agentId": AGENT_ID } },
          { $sort: { ts: -1 } },
          { $limit: 1 },
        ])
        .toArray();
  ...
  describe("Phase 3: Read-Path Quality", () => {
    it("all searchV2 scores are in [0,1] after RRF normalization", async () => {
      const { results, metadata } = await searchV2(
        ...
        AGENT_ID,
        {
          availablePaths: new Set<RetrievalPath>(["raw-window", "graph", "episodic"]),
    ...
    it("resultsByPath accurately reports per-path counts", async () => {
      ...
      const pathCountSum = Object.values(metadata.resultsByPath).reduce((s, c) => s + c, 0);
  ...
  describe("Phase 4: Index Health", () => {
    ...
    it("event time-range query uses timestamp index", async () => {
      ...
      const explainResult = await evtCol
        .find({
          agentId: AGENT_ID,
          timestamp: { $gte: new Date("2020-01-01"), $lte: new Date("2030-01-01") },
        })
        .explain("executionStats");
  ...
  describe("Phase 5: Cache Behavior", () => {
    ...
    const mockResults: MemorySearchResult[] = [
      ...
      },
      {
        ...
        startLine: 1,
        endLine: 3,
        snippet: "Prometheus metrics and Grafana dashboards for Kubernetes cluster monitoring",
        score: 0.82,
        source: "conversation",
      },
    ];
    ...
    it("reports exact cache hit after write", async () => {
      ...
      await waitForCache(db, PREFIX, { agentId: cacheAgentId, pathUsed: "hybrid" });
    ...
    it("derives TTL from source type: conversation=300s, kb=3600s", async () => {
      ...
      await waitForCache(db, PREFIX, { agentId: cacheAgentId, sourceScope: "reference" });
      ...
      const kbDoc = await cacheCol.findOne({ agentId: cacheAgentId, sourceScope: "reference" });
    ...
    it("increments hitCount and updates lastHitAt", async () => {
      ...
      const cacheCol = queryCacheCollection(db, PREFIX);
      const doc = await cacheCol.findOne({ agentId: cacheAgentId, pathUsed: "hybrid" });
  ...
  describe("Phase 6: Reranker Robustness", () => {
    const describeIfVoyage = VOYAGE_API_KEY ? describe : describe.skip;

    describeIfVoyage("with Voyage API key", () => {
      it("reranks valid results with Voyage API", async () => {
        const results: MemorySearchResult[] = [
          ...
          },
          {
            ...
            endLine: 0,
            score: 0.8,
            snippet: "JavaScript frontend React component library for the web dashboard",
            source: "conversation",
          },
          {
            path: "e/4",
      ...
      const results: MemorySearchResult[] = [
        {
          ...
          endLine: 0,
          score: 0.8,
          snippet: "Valid result with content about Kubernetes deployment strategies",
          source: "conversation",
        },
        { path: "e/2", startLine: 0, endLine: 0, score: 0.7, snippet: "", source: "conversation" },
        {
          path: "e/3",
          startLine: 0,
          endLine: 0,
  ...
  describe("Phase 7: Profile Synthesis", () => {
    it("returns complete profile with all sections populated", async () => {
      ...
      expect(
        profile.preferences.length + profile.decisions.length + profile.facts.length,
    ...
    it("ranks entities by relation count", async () => {
      ...
      for (let i = 1; i < profile.topEntities.length; i++) {
        expect(profile.topEntities[i - 1].relationCount).toBeGreaterThanOrEqual(
          profile.topEntities[i].relationCount,
        );
      }
    });
    ...
    it("emits profile-synthesis telemetry", async () => {
      ...
      const telCol = telemetryCollection(db, PREFIX);
      const docs = await telCol
        .find({ "meta.operation": "profile-synthesis", "meta.agentId": AGENT_ID })
        .toArray();
  ...
  describe("Phase 8: Telemetry Completeness", () => {
    it("getLatencyStats returns valid p50/p95/p99 via $percentile", async () => {
      ...
      await waitForTelemetry(db, PREFIX, { "meta.operation": "search", "meta.agentId": AGENT_ID });
    ...
    it("telemetry covers all operation types from prior phases", async () => {
      ...
      const operations = await telCol
        .aggregate([
          { $match: { "meta.agentId": AGENT_ID, ts: { $gte: new Date(Date.now() - 300_000) } } },
  ...
  describe("Phase 10: Graceful Degradation", () => {
    ...
    it("searchV2 returns empty on completely empty agent", async () => {
      const { results, metadata } = await searchV2(
        ...
        emptyAgentId,
        {
          availablePaths: new Set<RetrievalPath>(["raw-window", "graph", "episodic", "structured"]),
          hasGraphData: false,
          hasEpisodes: false,
          maxResults: 10,
        },
    ...
    it("reranker degrades gracefully with empty API key", async () => {
      const result = await crossEncoderRerank({
        ...
        query: "test",
        results: [
          { path: "p", startLine: 0, endLine: 0, snippet: "a", score: 0.9, source: "conversation" },
          {
            path: "p2",
            startLine: 0,
            endLine: 0,
        ...
        config: { enabled: true, model: "rerank-2.5", topN: 20, minScore: 0, voyageApiKey: "" },
      });
  ...
  describe("Phase 11: Score Normalization Deep Check", () => {
    ...
    it("raw-window path caps results at 50", async () => {
      const { metadata } = await searchV2(db, PREFIX, "everything about the project", AGENT_ID, {
        availablePaths: new Set<RetrievalPath>(["raw-window"]),
        maxResults: 100,
      });
  ...
  describe("Phase 12: MongoDB Operator Inventory", () => {
    it("$facet: returns structured results from parallel branches", async () => {
      ...
      const result = await smCol
        .aggregate([
          ...
          {
            $facet: {
              preferences: [{ $match: { type: "preference" } }, { $limit: 5 }],
    ...
    it("bulkWrite (updateOne + upsert): batch entity operations", async () => {
      ...
      const bulkOps = [
        {
          updateOne: {
            filter: { entityId: `bulk-test-1-${AGENT_ID}`, agentId: AGENT_ID },
            update: {
              $set: {
              ...
              $setOnInsert: { entityId: `bulk-test-1-${AGENT_ID}`, createdAt: new Date() },
            },
            upsert: true,
          },
        },
        {
          updateOne: {
            filter: { entityId: `bulk-test-2-${AGENT_ID}`, agentId: AGENT_ID },
            update: {
              $set: {
              ...
              $setOnInsert: { entityId: `bulk-test-2-${AGENT_ID}`, createdAt: new Date() },
            },
            upsert: true,
          },
        },
        {
          updateOne: {
            filter: { entityId: `bulk-test-3-${AGENT_ID}`, agentId: AGENT_ID },
            update: {
              $set: {
              ...
              $setOnInsert: { entityId: `bulk-test-3-${AGENT_ID}`, createdAt: new Date() },
            },
            upsert: true,
          },
        },
    ...
    it("createIndex (text): verifies text index on chunks", async () => {
      ...
      const hasTextIndex = indexes.some(
        (idx) => idx.key && Object.values(idx.key as Record<string, unknown>).includes("text"),
      );
    ...
    it("createIndex (sparse): creates with sparse option", async () => {
      ...
      const sparseIdx = indexes.find((i) => i.name === `${PREFIX}test_sparse_idx`);
      expect(sparseIdx).toBeDefined();
      expect(sparseIdx!.sparse).toBe(true);
    });

    const describeIfVoyageP12 = VOYAGE_API_KEY ? describe : describe.skip;

    describeIfVoyageP12("$vectorSearch operator", () => {
    ...
    describeIfVoyageP12("$search operator", () => {
  ...
  describe("Phase 13: Scale Stress Test", () => {
    it("handles 200+ events without OOM or timeout", async () => {
      const topics = ["auth", "database", "deployment", "testing", "monitoring"];
      ...
      const result = await searchV2(db, PREFIX, "auth database deployment", AGENT_ID, {
        availablePaths: new Set<RetrievalPath>(["raw-window", "episodic"]),
        hasEpisodes: true,
        maxResults: 30,
      });
  ...
  describe("Phase 14: Mutation Audit Trail", () => {
    ...
    it("mutation TTL index exists with expireAfterSeconds=7776000", async () => {
      const indexes = await mutationsCollection(db, PREFIX).listIndexes().toArray();
  ...
  describe("Phase 16: Procedural Memory Evolution", () => {
    ...
    it("evolveProcedure bumps version and records history", async () => {
      ...
      expect(doc!.steps).toEqual(["Build", "Lint", "Test", "Deploy", "Verify"]);
      expect(doc!.evolutionHistory).toBeDefined();
      expect(doc!.evolutionHistory.length).toBeGreaterThanOrEqual(1);
      const lastEntry = doc!.evolutionHistory[doc!.evolutionHistory.length - 1];
