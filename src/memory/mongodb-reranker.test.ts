...
function mockFetchSuccess(data: Array<{ index: number; relevance_score: number }>) {
...
describe("crossEncoderRerank", () => {
  ...
  it("prepends instruction to query when config.instruction is set", async () => {
    const results = makeResults(2);
    const config = makeConfig({
      instruction: "This is agent conversation memory. Prioritize recent results.",
    });
  ...
  it("filters out results with empty/blank snippets before sending to API", async () => {
    ...
    expect(body.documents).toEqual(["Alice works on ProjectX", "Bob manages TeamY"]);
