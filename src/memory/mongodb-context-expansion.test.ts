...
describe("expandSearchContext", () => {
  it("returns original results when no event-based chunks present", async () => {
    ...
    const results = [makeResult({ path: "kb/doc1", source: "reference", score: 0.9 })];
  ...
  it("fetches neighbor events for event-based chunks with sessionId", async () => {
    ...
    const results = [
      makeResult({ path: "events/mid", sessionId: "s1", timestamp: ts, score: 0.9 }),
    ];
    const expanded = await expandSearchContext({
      db,
      prefix: "test_",
  ...
  it("assigns neighbor score as parentScore * 0.95", async () => {
    ...
    const results = [
      makeResult({ path: "events/mid", sessionId: "s1", timestamp: ts, score: 0.8 }),
    ];
    const expanded = await expandSearchContext({
      db,
      prefix: "test_",
  ...
  it("deduplicates neighbors already in results", async () => {
    ...
    const results = [
      makeResult({ path: "events/a", sessionId: "s1", timestamp: ts1, score: 0.9 }),
      makeResult({ path: "events/b", sessionId: "s1", timestamp: ts2, score: 0.8 }),
    ];
    const expanded = await expandSearchContext({
      db,
      prefix: "test_",
  ...
  it("drops lowest-scored tail when neighbors would exceed maxResults", async () => {
    ...
    const results = [
      makeResult({ path: "events/mid", sessionId: "s1", timestamp: ts, score: 0.9 }),
      makeResult({ path: "kb/low", source: "reference", score: 0.1 }),
    ];
    const expanded = await expandSearchContext({
      db,
      prefix: "test_",
  ...
  it("handles events at session boundaries (no prior/next)", async () => {
    ...
    const results = [
      makeResult({ path: "events/first", sessionId: "s1", timestamp: ts, score: 0.9 }),
    ];
    const expanded = await expandSearchContext({
      db,
      prefix: "test_",
  ...
  it("does not expand non-event results (episodes, kb, etc.)", async () => {
    const { db } = createMockDb([]);
    const results = [
      makeResult({ path: "episode/a", source: "conversation", sessionId: "s1", score: 0.9 }),
      makeResult({ path: "kb/doc", source: "reference", score: 0.8 }),
    ];
    const expanded = await expandSearchContext({
      db,
      prefix: "test_",
