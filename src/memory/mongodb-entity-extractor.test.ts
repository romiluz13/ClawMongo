...
describe("mongodb-entity-extractor", () => {
  describe("RegexEntityExtractor", () => {
    ...
    it("extracts @mentions as person entities", async () => {
      const result = await extractor.extract("Talked to @alice about the project");
      expect(result).toContainEqual(
        expect.objectContaining({ name: "alice", type: "person", extractionMethod: "regex" }),
      );
    });

    it("extracts #tags as topic entities", async () => {
      const result = await extractor.extract("Working on #frontend #refactor today");
    ...
    it("extracts URLs as document entities", async () => {
      const result = await extractor.extract("See https://example.com/docs for details");
      expect(result).toContainEqual(
    ...
    it("extracts file paths as document entities", async () => {
      const result = await extractor.extract("Modified src/memory/mongodb-graph.ts");
      expect(result).toContainEqual(
        expect.objectContaining({
    ...
    it("extracts quoted names as person entities", async () => {
      const result = await extractor.extract('Meeting with "John Smith" about the design');
      expect(result).toContainEqual(
        expect.objectContaining({
          name: "John Smith",
          type: "person",
    ...
    it("deduplicates entities by name+type", async () => {
      ...
      const aliceEntities = result.filter((e) => e.name === "alice" && e.type === "person");
      expect(aliceEntities).toHaveLength(1);
    });

    it("returns empty array for content with no matches", async () => {
      const result = await extractor.extract("Just a plain message with nothing");
  ...
  describe("LLMEntityExtractor", () => {
    ...
    it("parses JSON array response correctly", async () => {
      const llmFn = vi
        .fn()
        .mockResolvedValue(JSON.stringify([{ name: "GitHub", type: "system", confidence: 0.8 }]));
      const extractor = new LLMEntityExtractor(llmFn, 5000);
    ...
    it("falls back to regex on timeout", async () => {
      ...
      expect(result).toContainEqual(
        expect.objectContaining({ name: "bob", type: "person", extractionMethod: "regex" }),
      );
    });
    ...
    it("handles markdown-wrapped JSON response", async () => {
      ...
      expect(result).toContainEqual(
        expect.objectContaining({ name: "React", type: "system", extractionMethod: "llm" }),
      );
    });
  });
  ...
  describe("RegexEntityExtractor — temporal grounding (Phase 7)", () => {
    ...
    it("extracts ISO dates as concept entities", async () => {
      const result = await extractor.extract("The meeting was on 2023-05-07 at the office");
      const dateEntities = result.filter((e) => e.name === "2023-05-07" && e.type === "concept");
    ...
    it("extracts US format dates (M/D/YYYY) as concept entities", async () => {
      const result = await extractor.extract("Deadline is 5/7/2023 for the project");
      const dateEntities = result.filter((e) => e.name === "5/7/2023" && e.type === "concept");
      expect(dateEntities).toHaveLength(1);
    });

    it("does not extract temporal entities when no dates present", async () => {
      const result = await extractor.extract("Just a plain message with no dates");
  ...
  describe("buildAssistantExtractionPrompt (Phase 8)", () => {
    it("includes assistant-specific focus areas", () => {
      const prompt = buildAssistantExtractionPrompt("I used the vector search tool");
  ...
  describe("LLMEntityExtractor — role-based prompts (Phase 8)", () => {
    ...
    it("sourceRole validated on entity (Phase 8 — schema)", async () => {
      const llmFn = vi
        .fn()
        .mockResolvedValue(JSON.stringify([{ name: "MongoDB", type: "system", confidence: 0.9 }]));
      const extractor = new LLMEntityExtractor(llmFn, 5000);
