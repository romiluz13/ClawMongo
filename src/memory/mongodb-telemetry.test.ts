...
describe("getLatencyStats", () => {
  ...
  it("filters by operation when provided", async () => {
    ...
    const [pipeline] = vi.mocked(mockCol.aggregate).mock.calls[0];
    const matchStage = (pipeline as Record<string, unknown>[])[0].$match as Record<string, unknown>;
...
describe("getOperationDistribution", () => {
  ...
  it("rounds avgDurationMs to integer", async () => {
    const toArrayFn = vi.fn().mockResolvedValue([{ _id: "search", count: 1, avgDurationMs: 99.9 }]);
    vi.mocked(mockCol.aggregate).mockReturnValue({ toArray: toArrayFn } as never);
