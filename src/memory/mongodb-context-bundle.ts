...
import { summarizeResultTrust } from "./mongodb-result-trust.js";
...
function clampTokenBudget(tokenBudget?: number): number {
  ...
  return Math.max(MIN_TOKEN_BUDGET, Math.min(MAX_TOKEN_BUDGET, Math.floor(tokenBudget ?? 0)));
}

function clampCount(value: number | undefined, defaultValue: number, maxValue: number): number {
...
function getSourceEventIds(doc: { sourceEventIds?: unknown }): string[] | undefined {
...
function summarizeSources(items: Array<{ source?: string }>): string {
  const unique = Array.from(
    new Set(
      items.map((item) => item.source?.trim()).filter((value): value is string => Boolean(value)),
    ),
  );
...
function renderItem(item: MemoryContextBundleSectionItem): string {
  ...
  const trustLabel = item.trust?.score;
  ...
  if (typeof trustLabel === "number") {
    line += ` {trust:${trustLabel.toFixed(2)}}`;
  }
  if (pathLabel) {
    line += ` (${pathLabel})`;
  }
...
async function loadEpisodeSummary(params: {
  ...
  return (await episodesCollection(db, prefix)
    .find(filter, { sort: { "timeRange.end": -1, updatedAt: -1 } })
    .limit(1)
    .project({
      episodeId: 1,
      title: 1,
    ...
    .next()) as EpisodeSummaryDoc | null;
}

function buildQueryEvidenceSection(searchResult: SearchBundleResult): CandidateSection | null {
  if (searchResult.results.length === 0) {
    return null;
  }
  const items = searchResult.results.map((result) => ({
  ...
  return {
    ...
    summary: `Retrieved ${items.length} evidence item${items.length === 1 ? "" : "s"} across ${sourceSummary}.`,
    items,
  };
}

function buildSummarySection(summaryDoc: EpisodeSummaryDoc): CandidateSection | null {
  const chosenSummary =
    ...
    items: [
      {
        ...
        path: summaryDoc.episodeId ? `episode:${summaryDoc.episodeId}` : undefined,
        source: "episodic",
...
function buildRecentEventsSection(
  ...
  return {
    ...
    items: eventDocs.map((doc) => ({
      ...
      path: typeof doc.eventId === "string" ? `events/${doc.eventId}` : undefined,
      source: "conversation",
      canonicalId: typeof doc.eventId === "string" ? `event:${doc.eventId}` : undefined,
      timestamp: doc.timestamp instanceof Date ? doc.timestamp : undefined,
      scope: typeof doc.scope === "string" ? (doc.scope as MemoryScope) : undefined,
      scopeRef: typeof doc.scopeRef === "string" ? doc.scopeRef : undefined,
      sourceEventIds: typeof doc.eventId === "string" ? [doc.eventId] : undefined,
      metadata: typeof doc.role === "string" ? { role: doc.role } : undefined,
    })),
  };
}

function buildDiscoverySection(projection: MemoryDiscoveryProjection): CandidateSection | null {
...
function buildProfileSection(profile: ProfileSynthesis): CandidateSection | null {
...
export async function buildContextBundle(params: {
  ...
  const tokenBudget = clampTokenBudget(request.tokenBudget);
  const maxActiveItems = clampCount(request.maxActiveItems, DEFAULT_MAX_ACTIVE_ITEMS, 6);
  const maxEvidenceItems = clampCount(
    request.maxEvidenceItems,
    DEFAULT_MAX_EVIDENCE_ITEMS,
    MAX_EVIDENCE_ITEMS,
  );
  const maxRecentEvents = clampCount(
  ...
  const [activeSlateResult, searchResult, summaryResult, recentEventsResult] = await Promise.all([
  ...
  if (searchResult.value) {
    ...
    if (searchResult.value.results.length > 0) {
      const section = buildQueryEvidenceSection(searchResult.value);
      if (section) {
  ...
  if (recentEventsResult.value) {
    ...
    const section = buildRecentEventsSection(sessionId, recentEventsResult.value);
    if (section) {
      candidates.push(section);
    }
  }

  if (request.includeDiscoveryProjection) {
  ...
  for (const candidate of candidates) {
    const section = materializeSection(candidate, Math.max(0, tokenBudget - estimatedTokensUsed));
    if (!section) {
      truncated = true;
      continue;
    }
  ...
  const trustSummary =
    ...
    (searchResult.value?.results.length
      ? summarizeResultTrust(searchResult.value.results)
      : undefined);
