...
export const FIELD_HELP: Record<string, string> = {
  ...
  "agents.defaults.memorySearch":
    "MongoDB-backed vector search over runtime memory sources (per-agent overrides supported).",
  ...
  "agents.defaults.memorySearch.sources":
    'Chooses which sources are indexed: "memory" reads workspace memory files, and "sessions" includes transcript history. Keep ["memory"] unless you need recall from prior chat transcripts.',
  ...
  "agents.defaults.memorySearch.store.path":
    "Legacy store path setting (unused in ClawMongo — MongoDB is the canonical store).",
  "agents.defaults.memorySearch.store.vector.enabled":
    "Legacy vector store setting (unused in ClawMongo — vector search is handled by MongoDB Atlas Search / mongot).",
  "agents.defaults.memorySearch.store.vector.extensionPath":
    "Legacy vector extension path (unused in ClawMongo).",
  ...
  "agents.defaults.memorySearch.cache.enabled":
    "Caches computed chunk embeddings so reindexing and incremental updates run faster (default: true). In ClawMongo, the embedding cache is stored in MongoDB.",
  memory:
    "Memory backend configuration (global). ClawMongo uses MongoDB as the only canonical runtime memory backend.",
  "memory.backend": 'ClawMongo supports only "mongodb". This is the default and only valid value.',
  "memory.citations":
  ...
  "agents.defaults.memorySearch.cache.maxEntries":
    "Sets a best-effort upper bound on cached embeddings for memory search. In ClawMongo, the embedding cache is stored in MongoDB.",
