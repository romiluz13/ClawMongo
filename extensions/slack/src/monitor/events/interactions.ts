    ...
    async (args: SlackActionMiddlewareArgs) => {
      ...
      if (pluginInteractionData) {
        ...
        const pluginResult = await dispatchPluginInteractiveHandler({
          ...
          respond: {
            acknowledge: async () => {},
            reply: async (params: { text: string; responseType?: "ephemeral" | "in_channel" }) => {
              if (!respond) {
                return;
              }
              await respond({
                text: params.text,
                response_type: params.responseType ?? "ephemeral",
              });
            },
            ...
            editMessage: async (params: { text?: string; blocks?: unknown[] }) => {
              if (!channelId || !messageTs) {
                return;
              }
              await ctx.app.client.chat.update({
                channel: channelId,
                ts: messageTs,
                text: params.text ?? typedBody.message?.text ?? "",
                ...(Array.isArray(params.blocks)
                  ? { blocks: params.blocks as (Block | KnownBlock)[] }
                  : {}),
              });
            },
          },
