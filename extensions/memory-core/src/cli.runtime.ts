...
async function summarizeQmdIndexArtifact(manager: MemoryManager): Promise<string | null> {
  ...
  if (!status || (status as { backend: string }).backend !== "qmd") {
    return null;
  }
  const dbPath = (status as Record<string, unknown> & { dbPath?: string }).dbPath?.trim();
  if (!dbPath) {
    return null;
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
      ...
      run: async (manager) => {
        ...
        const scan = workspaceDir
          ? await scanMemorySources({
              ...
              agentId,
              sources,
              extraPaths: (status as Record<string, unknown>).extraPaths as string[] | undefined,
            })
          : undefined;
  ...
  for (const result of allResults) {
    ...
    const storePath = (status as Record<string, unknown> & { dbPath?: string }).dbPath
      ? shortenHomePath((status as Record<string, unknown> & { dbPath?: string }).dbPath!)
      : "<unknown>";
    ...
    const extraPaths = status.workspaceDir
      ? formatExtraPaths(
          status.workspaceDir,
          ((status as Record<string, unknown>).extraPaths as string[] | undefined) ?? [],
        )
      : [];
    const lines = [
    ...
    if (status.sourceCounts?.length) {
      ...
      for (const entry of status.sourceCounts) {
        const total = scan?.sources?.find(
          (scanEntry) => (scanEntry.source as string) === (entry.source as string),
        )?.totalFiles;
        const counts =
          total === null
    ...
    if ((status as Record<string, unknown>).fallback) {
      lines.push(
        `${label("Fallback")} ${warn(((status as Record<string, unknown>).fallback as { from: string }).from)}`,
      );
    }
    if (status.vector) {
      ...
      if ((status.vector as Record<string, unknown>).extensionPath) {
        lines.push(
          `${label("Vector path")} ${info(shortenHomePath((status.vector as Record<string, unknown>).extensionPath as string))}`,
        );
      }
    ...
    if (
      (status as unknown as Record<string, unknown> & { fallback?: { reason?: string } }).fallback
        ?.reason
    ) {
      lines.push(
        muted(
