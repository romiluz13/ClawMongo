# Upstream Sync Wave 25

## Scope

- Range: `bbf3b4acf2...7a09255361`
- Why this wave next: the sync report shows no protected MongoDB-first hotspots and no excluded backend paths, even though the range carries meaningful plugin/runtime refactors.
- Goal: absorb provider discovery and auth-choice contract work, lazy runtime loading, and the plugin-owned session/outbound seam cleanup without touching MongoDB memory truth.
...
Wave 25 is only complete when:

- the range `bbf3b4acf2...7a09255361` is merged onto a clean branch
